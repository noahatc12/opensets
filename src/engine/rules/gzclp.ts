 ...
 *
 * Working increment is a fixed microloading default (2.5 kg ≈ 5 lb); per-lift
...
/** GZCLP working increment (kg). Microloading default; upper/lower split is the generator's job. */
const INC = 2.5;
...
function buildStage(spec: StageSpec, weightKg: number, amrapLast: boolean): PrescribedSet[] {
...
function tier12(
  ...
  if (work.length === 0) {
    const w = roundLoad(state.workingWeightKg, settings);
    return {
      prescription: {
        sets: buildStage(table[stage]!, w, amrapLast),
        reason: `${label} — starting at ${fmt(w)} kg.`,
        flags,
      },
      nextState: { ...state, workingWeightKg: w, stage, anchorKg: state.anchorKg ?? w },
    };
  }
  ...
  let nextStage = stage;
  let nextWeight = state.workingWeightKg;
  let anchor = state.anchorKg ?? state.workingWeightKg;
  let reason: string;

  if (success) {
    nextWeight = state.workingWeightKg + INC;
    if (stage === 0) anchor = nextWeight; // track the 3×10 (or 5×3) weight for the reset
    reason = `+${fmt(INC)} kg — completed ${spec.sets}×${spec.reps}.`;
  } else if (stage < 2) {
    nextStage = stage + 1;
    flags.push('stageChange');
    reason = `Missed — drop to ${table[nextStage]!.sets}×${table[nextStage]!.reps}, same weight.`;
  } else if (label === 'T1') {
    nextWeight = roundLoad(0.85 * (bestSessionE1rm(work) ?? state.workingWeightKg), settings);
    nextStage = 0;
    flags.push('stageChange');
    reason = `${label} stall — recalibrate to ${fmt(roundLoad(nextWeight, settings))} kg (≈85%), restart stage 1.`;
  } else {
    nextWeight = anchor + INC;
    anchor = nextWeight;
    nextStage = 0;
    flags.push('stageChange');
    reason = `${label} stall — restart 3×10 at ${fmt(roundLoad(nextWeight, settings))} kg.`;
  }

  const rw = roundLoad(nextWeight, settings);
  return {
    prescription: { sets: buildStage(table[nextStage]!, rw, amrapLast), reason, flags },
    nextState: { ...state, workingWeightKg: rw, stage: nextStage, anchorKg: anchor },
  };
}
  ...
  if (work.length === 0) {
    ...
    return {
      prescription: {
        ...
        reason: `T3 — starting at ${fmt(w)} kg.`,
        flags: [],
      },
      nextState: { ...state, workingWeightKg: w },
    };
  }
  const amrapSet = work[work.length - 1]!;
  const hit = amrapSet.completed && amrapSet.reps >= 25;
  const nextWeight = hit ? state.workingWeightKg + INC : state.workingWeightKg;
  const rw = roundLoad(nextWeight, settings);
  return {
    prescription: {
      sets: buildStage(T3, rw, true),
      reason: hit
        ? `+${fmt(INC)} kg — ${amrapSet.reps} reps on the AMRAP set (≥ 25).`
        : `Hold ${fmt(rw)} kg — build the AMRAP set toward 25 reps.`,
      flags: [],
    },
    nextState: { ...state, workingWeightKg: rw },
  };
}
