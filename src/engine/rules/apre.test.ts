...
const amrap = (reps: number, completed = true): SetResult[] => [
  { weightKg: 50, reps: 10, type: 'working', completed: true },
  { weightKg: 75, reps: 6, type: 'working', completed: true },
  { weightKg: 100, reps, type: 'amrap', completed },
];
...
describe('APRE', () => {
  ...
  it('APRE-10: many AMRAP reps raise the RM', () => {
    ...
    expect(r.nextState.workingWeightKg).toBe(105);
    expect(r.prescription.reason).toMatch(/\+5 kg/);
  });

  it('APRE-10: target-band reps hold the RM', () => {
    const r = apreNext(rule(10), state(100), amrap(10), settings); // 9–11 → 0
    expect(r.nextState.workingWeightKg).toBe(100);
    expect(r.prescription.reason).toMatch(/Hold/);
  });

  it('APRE-10: low AMRAP reps drop the RM', () => {
    const r = apreNext(rule(10), state(100), amrap(5), settings); // 4–6 → −5
    expect(r.nextState.workingWeightKg).toBe(95);
    expect(r.prescription.reason).toMatch(/−5 kg/);
  });
  ...
  it('APRE-6 and APRE-3 use their own warm-up reps and tables', () => {
    ...
    expect(r6.nextState.workingWeightKg).toBe(105);
    const r3 = apreNext(rule(3), state(100), amrap(5), settings); // 5–6 → +5
    expect(r3.prescription.sets[0]!.targetReps).toBe(3);
    expect(r3.nextState.workingWeightKg).toBe(105);
  });

  it('an uncompleted AMRAP set counts as 0 reps (RM drops)', () => {
    const r = apreNext(rule(6), state(100), amrap(9, false), settings); // 0 → −5
    expect(r.nextState.workingWeightKg).toBe(95);
  });

  it('falls back to the last working set when no AMRAP set is logged', () => {
    const noAmrap: SetResult[] = [
      { weightKg: 100, reps: 4, type: 'working', completed: true },
    ];
    const r = apreNext(rule(6), state(100), noAmrap, settings); // 4 → −2.5
    expect(r.nextState.workingWeightKg).toBe(97.5);
  });

  it('very high AMRAP reps give the largest bump', () => {
    const r = apreNext(rule(3), state(100), amrap(9), settings); // 7+ → +7.5
    expect(r.nextState.workingWeightKg).toBe(107.5);
  });
});
